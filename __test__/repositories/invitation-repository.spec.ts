...
import { GET_SINGLE_CUSTOMER_BY_ID_1, GET_SINGLE_CUSTOMER_BY_ID_2, GET_SINGLE_CUSTOMER_BY_ID_3, GET_USERS_WITHIN_0_KM, GET_USERS_WITHIN_100_KM, GET_USERS_WITHIN_500_KM, GET_USERS_WITHIN_50_KM, GET_USERS_WITH_RANDOM_DATA, GET_USERS_WITH_WRONG_DATA } from '../factories/invitations.repository.factory';
describe('intersection service', () => {
    ...
    describe('repositories tests (customers list based on distance)',()=>{
        it('should retrive a list of users within 100km', async () => {
            const result=await repository.findAll(GET_USERS_WITHIN_100_KM,new Location(Number('52.493256'),Number('13.446082')));
            expect(result.length).toEqual(19);
        
        });
        it('should retrive a list of users within 50km', async () => {
            const result=await repository.findAll(GET_USERS_WITHIN_50_KM,new Location(Number('52.493256'),Number('13.446082')));
            expect(result.length).toEqual(7);
        
        });
        it('should no user outside 0km', async () => {
            const result=await repository.findAll(GET_USERS_WITHIN_0_KM,new Location(Number('52.493256'),Number('13.446082')));
            expect(result.length).toEqual(0);
        });
        it('should all users within 500km', async () => {
            const result=await repository.findAll(GET_USERS_WITHIN_500_KM,new Location(Number('52.493256'),Number('13.446082')));
        ...
        it('should be defined', async () => {
            const result=await repository.findAll(GET_USERS_WITH_RANDOM_DATA,new Location(Number('54.6931541546853256'),Number('44.48741231546082')));
            expect(result.length).toEqual(0);
        });
