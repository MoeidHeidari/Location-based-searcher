import { ArgumentsHost, Catch, ExceptionFilter, Injectable, InternalServerErrorException, NotFoundException } from "@nestjs/common";
...
export class InvitationRepository{
    ...
    async findAll(query?:InvitationRquestDTO,current_location?:Location):Promise<Customer[]>{
        const result= await asyncFilter(Customers, async (customer: Customer) => {
            return (await distance(Number(customer.location?.lat),Number(customer.location?.long),Number(current_location?.lat),Number(current_location?.long))<=(Number(query?.radius)))
        });
